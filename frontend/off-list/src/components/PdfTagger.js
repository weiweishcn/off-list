...
// Room type colors mapping (keep existing colors)
const roomColors = {
...
const PdfTagger = ({ 
    ...
    const loadFloorPlan = async () => {
      ...
      try {
        ...
        if (isPDF) {
          ...
          console.log('Starting to load image from URL:', floorPlanUrl);
          ...
          img.onload = () => {
            ...
            if (containerRef.current) {
              ...
              console.log('Setting stage size:', {
                newWidth,
                newHeight,
          ...
          img.onerror = (err) => {
            ...
            setError('Failed to load image. Please check if the image URL is accessible and the format is supported.');
            setLoading(false);
          };
  ...
  const handleWheel = (e) => {
    ...
    // Recalculate room counts after removing a tag
    const newCounts = {};
  ...
  const handleStageClick = (e) => {
    const getNextRoomNumber = (baseType) => {
      const currentCount = roomCounts[baseType] || 0;
    ...
    const nextNumber = getNextRoomNumber(baseType);
    ...
    <div className="space-y-4">
      <div className="bg-white p-4 rounded-lg shadow-sm">
        ...
        {floorPlanUrl.toLowerCase().endsWith('.pdf') ? (
          <ReactPdfViewer pdfUrl={floorPlanUrl} />
        ) : (
          <div 
            ref={containerRef} 
          ...
          >
            <Stage
              ref={stageRef}
              width={stageSize.width || window.innerWidth * 0.9}
              height={stageSize.height || window.innerHeight * 0.8}
